...
const countByValue = (values: (string | null | undefined)[]) => {
  ...
  return Array.from(counts.entries())
    ...
    .sort(
      (first, second) =>
        second.count - first.count || first.value.localeCompare(second.value)
    );
};
...
export const getSessionAnalytics = async (params: {
  ...
  return {
    ...
    evaluations: {
      ...
      rolling: session.evaluations.filter(item => item.type === 'ROLLING')
        .length,
...
export const getUserProgress = async (params: {
  ...
  const finalEvaluations = sessions
    .map(session => ({
      session,
      evaluation: getLatestFinalEvaluation(session.evaluations),
    }))
...
export const formatEvaluationExportMarkdown = (
  exportData: NonNullable<Awaited<ReturnType<typeof getFinalEvaluationExport>>>
): string => {
